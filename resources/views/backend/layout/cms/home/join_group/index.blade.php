...
@section('title', 'Update Home Join Group Content')
    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Update Home Join Group</h4>
                        <form id="my-form" class="forms-sample"
                            action="{{ route('admin.cms.home.how.join.group.update.content') }}" method="POST"
                                ...
                                <div class="col-md-5 col-12">
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <textarea type="text" class="form-control @error('content') is-invalid @enderror" id="content" name="content"
                                            placeholder="Content Here...">{{ old('content') ?? ($group->content ?? '') }}</textarea>
                                        @error('content')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="file"
                                            data-default-file="{{ !empty($group->image) && file_exists(public_path($group->image)) ? url($group->image) : url('backend/images/image-not.png') }}"
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control @error('btn_text') is-invalid @enderror"
                                            ...
                                            value="{{ old('btn_text') ?? ($group->btn_text ?? '') }}">
                                        @error('btn_text')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-7 col-12 d-flex flex-column">
                                    <div class="card-body flex-grow-1">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <h4 class="card-title mb-3">How To Join List</h4>
                                            <a href="{{ route('admin.cms.home.how.join.group.create') }}"
...
@push('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if (!$.fn.DataTable.isDataTable('#data-table')) {
                let dTable = $('#data-table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('admin.cms.home.how.join.group.index') }}",
                        type: "get",
                    },
        ...
        const deleteAuction = (id) => {
            try {
                let url = '{{ route('admin.cms.home.how.join.group.destroy', ':id') }}';
